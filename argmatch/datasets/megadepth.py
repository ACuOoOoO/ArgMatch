...
from argmatch.utils import get_depth_tuple_transform_ops, get_tuple_transform_ops
import argmatch
...
class MegadepthScene:
    def __init__(
        ...
        self.rand_shake_flag = False if shake_t<1 else True
        ...
        if 'pairs' in scene_info.keys():
            self.pairs = scene_info["pairs"]
        if 'pair_infos' in scene_info.keys():
            self.pairs = scene_info["pair_infos"]
        if 'overlaps' in scene_info.keys():
        ...
        self.colorjit = torchvision.transforms.ColorJitter(0.2,0.2,0.2) if colorjit else None
        self.suffix_transform_ops = tvt.Compose([tvt.ColorJitter(0.4,0.4,0.4),
                                                tvt.RandomAdjustSharpness(2.0, p=0.5)])
        
    def load_im(self, im_path):
        im = Image.open(im_path)
        return im
    
    def horizontal_flip(self, im_A, im_B,im_A_suffix, depth_A, depth_B,  K_A, K_B):
        im_A = im_A.flip(-1)
        im_B = im_B.flip(-1)
        im_A_suffix = im_A_suffix.flip(-1)
        depth_A, depth_B = depth_A.flip(-1), depth_B.flip(-1) 
        flip_mat = torch.tensor([[-1, 0, self.wt],[0,1,0],[0,0,1.]]).to(K_A.device)
        K_A = flip_mat@K_A  
        K_B = flip_mat@K_B  
        
        return im_A, im_B, im_A_suffix, depth_A, depth_B, K_A, K_B
    ...
    def scale_intrinsic(self, K, wi, hi):
        sx, sy = self.wt*1.0 / wi, self.ht*1.0 / hi
        sK = torch.tensor([[sx, 0, 0], [0, sy, 0], [0, 0, 1]])
        return sK @ K

    def rand_shake(self, imgs, depth):
        ...
        return [
            ...
            for thing in imgs
        ]+[tvf.affine(thing, angle=0.0, translate=list(t), scale=1.0, shear=[0.0, 0.0],fill=1e6)
            for thing in depth], t

    def __getitem__(self, pair_idx):
        if len(self.pairs[pair_idx])<3:
            idx1, idx2 = self.pairs[pair_idx]
        else:
            idx1, idx2 = self.pairs[pair_idx][0]
        ...
        T_2to1 = torch.tensor(np.matmul(T1, np.linalg.inv(T2)), dtype=torch.float)[
            :4, :4
        ]  # (4, 4)
        ...
        im_A_suffix = self.suffix_transform_ops(im_A)
        if self.colorjit is not None:
            if torch.rand(1)>0.7:
                im_A = self.colorjit(im_A)
            if torch.rand(1)>0.7:
                im_B = self.colorjit(im_B)
        depth_A, depth_B = self.depth_transform_ops(
            (depth_A[None, None], depth_B[None, None])
        )

        im_A, im_B = im_A[None], im_B[None]
        im_A_suffix = im_A_suffix[None]
        
        
        if self.rand_shake_flag & (torch.rand(1) > 0.1):
            [im_A, im_A_suffix, depth_A], t = self.rand_shake([im_A, im_A_suffix], [depth_A])
            K1[:2, 2] += t
        if self.rand_shake_flag & (torch.rand(1) > 0.1):
            [im_B, depth_B], t = self.rand_shake([im_B], [depth_B])
            K2[:2, 2] += t
        
        # if self.rand_shake_flag& (torch.rand(1) > 0.5):
        #     [im_A, im_A_suffix, im_B, depth_A, depth_B], t = self.rand_shake([im_A, im_A_suffix, im_B], [depth_A, depth_B])
        #     K1[:2, 2] += t
        #     # [im_B, depth_B], t = self.rand_shake([im_B], [depth_B])
        #     K2[:2, 2] += t
        ...
        if self.use_horizontal_flip_aug:
            if np.random.rand() > 0.5:
                im_A, im_B, im_A_suffix, depth_A, depth_B, K1, K2 = self.horizontal_flip(im_A, im_B, im_A_suffix, depth_A, depth_B, K1, K2)
        # im_B_suffix = self.suffix_transform_ops(im_B)
        
        data_dict = {
            "im_A": im_A[0],
            "im_A_suffix": im_A_suffix[0],
            "im_A_identifier": self.image_paths[idx1].split("/")[-1].split(".jpg")[0],
            "im_B": im_B[0],
            # "im_B_suffix": im_B_suffix[0],
